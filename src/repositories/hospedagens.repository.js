...
export async function getHospedagem(destino, min, max){
    ...
    SELECT hospedagem.id, hospedagem.nome, hospedagem.preco, hospedagem."fotoPrincipal", "cidadeDestino".nome AS cidade, MAX(hospedagem.preco) AS "maiorPreco"
    FROM hospedagem
    ...
    GROUP BY hospedagem.id, hospedagem.nome, hospedagem.preco, hospedagem."fotoPrincipal", "cidadeDestino".nome
  `, [destino, min, max])
}
